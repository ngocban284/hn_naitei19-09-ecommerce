...
//    CartDetail findByProductIdAndCart(Long productId, Cart cart);
    ...
    CartDetail findByProductIdAndCartId(Integer productId, Long cartId);
...
//    boolean existsByProduct_IdAndCart(Long productId, Cart cart);
    ...
    boolean existsByProductIdAndCart(@Param("productId") Integer productId, @Param("cart") Cart cart);

    //existsByProductIdAndCartId
    @Query("SELECT CASE WHEN COUNT(cd) > 0 THEN true ELSE false END FROM CartDetail cd WHERE cd.product.id = :productId AND cd.cart.id = :cartId")
    boolean existsByProductIdAndCartId(Integer productId, Long cartId);
